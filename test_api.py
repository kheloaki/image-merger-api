#!/usr/bin/env python3
"""
Test script for Image Merger API
"""
...
def test_api(base_url, model_image_path, product_image_path, 
    ...
    print(f"\n{'='*60}")
    print(f"Testing Image Merger API")
    print(f"{'='*60}")
    ...
    try:
        ...
        # Test merge endpoint
        print("\n2️⃣  Testing merge endpoint...")
        
        files = {
            'model_image': open(model_image_path, 'rb'),
        ...
        response = requests.post(f"{base_url}/merge", files=files, data=data)
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python test_api.py <model_image> <product_image> [base_url] [height] [format]")
        print("\nExample (local):")
        print("  python test_api.py model.jpg product.png")
        print("\nExample (Railway):")
        print("  python test_api.py model.jpg product.png https://your-app.railway.app 1200 jpg")
